...
import { SCHOOL_STATS } from '../../constants/mockData';
...
export default function AuthorityProfileScreen({ navigation }) {
  ...
  const ITEMS = [
    ...
    { icon: 'bar-chart-outline', label: 'School Analytics', color: COLORS.primary },
    { icon: 'document-outline', label: 'Generate Reports', color: COLORS.primary },
  ...
  return (
    <View style={{ flex: 1, backgroundColor: COLORS.background }}>
      ...
      <LinearGradient colors={[COLORS.primaryDark, COLORS.primary]} style={[styles.header, { paddingTop: insets.top + 20 }]}>
        ...
        <Text style={styles.userRole}>{user.position || 'School Authority'} • {SCHOOL_NAME}</Text>
      </LinearGradient>
      <ScrollView contentContainerStyle={[styles.body, { paddingBottom: insets.bottom + 24 }]}>
        <View style={styles.statsRow}>
          {[['Students', SCHOOL_STATS.totalStudents], ['Teachers', SCHOOL_STATS.totalTeachers], ['Parents', SCHOOL_STATS.totalParents]].map(([label, val]) => (
...
const styles = StyleSheet.create({
  ...
  statsRow: { flexDirection: 'row', backgroundColor: COLORS.surface, borderRadius: RADIUS.md, padding: SPACING.md, marginBottom: SPACING.md, ...SHADOW.sm },
