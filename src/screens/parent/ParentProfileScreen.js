...
import { View, Text, ScrollView, StyleSheet, StatusBar, TouchableOpacity, Alert, Image, Linking } from 'react-native';
...
import { APP_NAME, SCHOOL_NAME, APP_VERSION, SCHOOL_WEBSITE } from '../../constants/branding';
...
export default function ParentProfileScreen({ navigation }) {
  ...
  const openWebsite = async () => {
    const canOpen = await Linking.canOpenURL(SCHOOL_WEBSITE);
    if (canOpen) Linking.openURL(SCHOOL_WEBSITE);
    else Alert.alert('Unable to open link', SCHOOL_WEBSITE);
  };
  ...
  const MENU_ITEMS = [
    ...
    { icon: 'globe-outline', label: 'Visit School Website', color: COLORS.primary, onPress: openWebsite },
    { icon: 'shield-checkmark-outline', label: 'Privacy & Security', color: COLORS.primary, onPress: () => navigation.navigate('PrivacySecurity') },
    { icon: 'help-circle-outline', label: 'Help & Support', color: COLORS.primary, onPress: () => navigation.navigate('HelpSupport') },
    { icon: 'document-text-outline', label: 'Terms & Privacy Policy', color: COLORS.primary, onPress: () => navigation.navigate('TermsPrivacy') },
  ...
  return (
    <View style={{ flex: 1, backgroundColor: COLORS.background }}>
      ...
      <LinearGradient colors={[COLORS.primaryDark, COLORS.primary]} style={[styles.header, { paddingTop: insets.top + 20 }]}>
        ...
        <Text style={styles.userRole}>Parent • {SCHOOL_NAME}</Text>
        ...
        {children.map((child) => (
          <View key={child.id} style={styles.childRow}>
            <View style={styles.childAvatar}>
              <Text style={styles.childAvatarText}>{child.name.split(' ').map((n) => n[0]).join('').slice(0, 2)}</Text>
            </View>
            <View style={{ flex: 1 }}>
              <Text style={styles.childName}>{child.name}</Text>
              <Text style={styles.childMeta}>{child.class} • {child.admissionNo}</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: FONTS.sizes.sm, fontWeight: FONTS.weights.bold, color: COLORS.textSecondary, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: SPACING.sm, marginTop: SPACING.md },
  childRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.surface, borderRadius: RADIUS.md, padding: SPACING.md, marginBottom: SPACING.sm, gap: SPACING.md, ...SHADOW.sm },
  childAvatar: { width: 44, height: 44, borderRadius: 22, backgroundColor: COLORS.primaryLight, alignItems: 'center', justifyContent: 'center' },
  childAvatarText: { fontSize: FONTS.sizes.sm, fontWeight: FONTS.weights.bold, color: '#FFF' },
  childName: { fontSize: FONTS.sizes.md, fontWeight: FONTS.weights.bold, color: COLORS.text },
  childMeta: { fontSize: FONTS.sizes.xs, color: COLORS.textMuted },
